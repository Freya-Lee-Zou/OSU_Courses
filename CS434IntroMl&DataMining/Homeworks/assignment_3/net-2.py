...
# activation = "Sigmoid" if False else "ReLU"
activation = "ReLU" 
...
def main():
  ...
  KaggleResults = net.forward(X_test)
  outputR = np.argmax(KaggleResults, axis=1).astype(int)  
  # Add index to the array
  outputFormat = np.column_stack((np.arange(len(outputR)), outputR))
  # Add header
  header = np.array([["id", "digit"]])
  outputFormat = np.vstack((header, outputFormat))
  # Save to CSV file
  np.savetxt('SuzyRults.csv', outputFormat, fmt='%s', delimiter=',')
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    # dl/db = sigma(dl/dz @ dz/db)= Sigma(dl/dz)
    self.grad_bias = np.sum(grad, axis=0)[np.newaxis, :]
    # we have to return dl/dx here 
    # dl/dx = dl/dz W^T
