...
def main():
    ...
    for i in range(len(example_train_x)):
        #return true if [i] == nearest_neighbors,otherwirse nothing return
    ...
    for k in [1,3,5,7,9,59,60,61,67,99,999,8000]:
    # for k in range(55,66,1):
      t0 = time.time()
      ...
      #######################################
      train_acc = 0 
      traning_array = predict(train_X, train_y, train_X, k)
      train_acc = np.mean(traning_array == train_y)
...
def get_nearest_neighbors(example_set, query, k):
    #Calculte the distance between the query point and all points in the example_set.
    distances = np.linalg.norm(example_set - query, axis=1)
    #sort all distances in ascending order and sotre the k nearest neighbor in the
...
def knn_classify_point(examples_X, examples_y, query, k):
    # get the indeceis of all 
    count1 = 0
    count0 = 0
    index = get_nearest_neighbors(examples_X, query, k)
    for i in index:
        if examples_y[i] == 1:
            count1+=1
        elif examples_y[i]== 0:
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    # Step 1: Split data into training sets and testing sets
    splited_X = np.split(train_X, num_folds)
    splited_Y = np.split(train_y, num_folds)
    
    # Declare an empty list to store accuracy rates for each fold
    accuriesPerFold_a = []

    # Step 2: Loop through the folds
    for i in range(num_folds):
        # Take the current fold as testing data
        testing_qurreis_x = splited_X[i]
        testing_query_y = splited_Y[i]

        # Initialize empty arrays/lists for training data
        bufffer_fold_X = []
        buffer_fold_Y = []

        for j in range(num_folds):
            if i != j:
                bufffer_fold_X.extend(splited_X[j])
                buffer_fold_Y.extend(splited_Y[j])

        # Perform predictions and compute accuracy
        predictionresults = predict(bufffer_fold_X, buffer_fold_Y, testing_qurreis_x, k)
        accuracy = compute_accuracy(testing_query_y, predictionresults)
        
        # Append the accuracy to the list
        accuriesPerFold_a.append(accuracy)
        
    avg_val_acc = np.mean(accuriesPerFold_a)
    varr_val_acc = np.var(accuriesPerFold_a)
...
#                to the pC     redicted label of an example
#
# Output:
...
def load_data():
    ...
    #increase weighting of capital_gain atribute
    test_X[:,2] = test_X[:,2] * 52
    train_X[:,2] = train_X[:,2] * 52
