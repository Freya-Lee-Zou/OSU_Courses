...
/******************************************************************************
* enc_server code
* 1. Create a socket and connect to the client.
* 2. Receve plaintxt file and key from enc_client.c .
* 3. Encrept the plaintext with specified key and send back to the enc_client
*/
...
//This fucntion decreypt the message whcig received frim the enc_server
char *decryptPlaintext(char* buffer){
  int ASKII_A = 0, ASKII_B = 0;
   char *keyBuf = NULL;
   ...
   if (strcmp(verficationEnc,"start ") != 0) {
     return("You are not allowed to connect to the wrong client\n end \n");
   }else{
     plaintextEncypt = strtok(NULL,"\n" );
   //   printf("plaintxt in the sec_sever is %s",toDecrypt);
     keyBuf = strtok(NULL,"\n" );
      for (int i = 0; i < strlen(plaintextEncypt); i++) {
         //Get ascii value for a then set up to A
         ...
         //Subtract 65 from the abs of a
         ASKII_A = abs(ASKII_A) - 65; 
         //Subtract 65 from the abs of b
         ASKII_B = abs(ASKII_B) - 65; 
         ASKII_A -= ASKII_B;  //Add key and buffer value
         if (ASKII_A < 0 ){
            ASKII_A += 27;
         } 
         // get mod 26 of a
         ASKII_A = ASKII_A % 27; 
         if((ASKII_A + 65) > 90 || (ASKII_A + 65) < 65){
          ASKII_A = 32 - 65;
        }
        // store ascii value of a + 65 into array at i
         plaintextEncypt[i] = ASKII_A + 65; 
      }
   }
   // catenate end back to the decrepted txt 
...
int main(int argc, char *argv[]){
   ...
   while(1){
      ...
      if (pid_val == -1){
         ...
         fflush(stdout);
         strcpy(buffer,decryptPlaintext(buffer));
         fflush(stdout);
