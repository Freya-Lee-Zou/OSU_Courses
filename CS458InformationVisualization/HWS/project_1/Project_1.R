...
ggplot(data, aes(x = Base_Salary, fill = Gender)) +
  geom_histogram(bins = 30, alpha = 0.5, position = "identity") +  
  labs(title = "Gender-Based Salary Distribution",
       x = "Base Salary in USD",
       y = "Count") +
  facet_wrap(~ Gender) +
...
# ggplot(data, aes(x = Department_Name, y = Grade, fill = Base_Salary)) +
#   geom_tile() +
#   labs(title = "Salary Heatmap by Department and Grade",
#        x = "Department",
#        y = "Grade",
#        fill = "Base Salary") +
#   theme_minimal()
...
# plot(avg_salary$avg_salary, as.numeric(factor(avg_salary$Department_Name)),
#      main = "Correlation between Department and Average Salary",
#      xlab = "Average Salary in USD", ylab = "Department",
#      pch = 19, col = "blue")
